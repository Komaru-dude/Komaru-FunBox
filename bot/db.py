...
def sync_all():
    ...
    for table_name, columns in expected_tables.items():
        ...
        if not exists:
            column_defs = ",\n".join(
                [" ".join(col) if isinstance(col, tuple) else col for col in columns]
            )
            ...
            for col in columns:
                if (
                    ...
                    and col[0] not in existing
                    and not col[0].startswith("PRIMARY")
                ):
                    try:
                        cursor.execute(
                            f"ALTER TABLE {table_name} ADD COLUMN {col[0]} {col[1]}"
                        )
    ...
    for chat_id in chat_ids:
        cursor.execute(
            "SELECT feature_name FROM features WHERE chat_id = ?", (chat_id,)
        )
        ...
        for feature, enabled in default_features:
            if feature not in existing_features:
                cursor.execute(
                    "INSERT INTO features (chat_id, feature_name, is_enabled) VALUES (?, ?, ?)",
...
def init_chat_features(chat_id: int):
    ...
    for feature, enabled in default_features:
        cursor.execute(
            """INSERT OR IGNORE INTO features (chat_id, feature_name, is_enabled) VALUES (?, ?, ?)""",
            (chat_id, feature, enabled),
        )
    conn.commit()
    conn.close()
...
def is_feature_enabled(chat_id: int, feature_name: str) -> bool:
    ...
    cursor = conn.cursor()
    cursor.execute(
        """SELECT is_enabled FROM features WHERE chat_id = ? AND feature_name = ?""",
...
def is_feature_exists(chat_id: int, feature_name: str) -> bool:
    with sqlite3.connect(DB_PATH) as conn:
        result = conn.execute(
            """SELECT 1 FROM features WHERE chat_id = ? AND feature_name = ? LIMIT 1""",
            (chat_id, feature_name),
        ).fetchone()
    return True if result else False
...
def enable_feature(chat_id: int, feature_name: str):
    with sqlite3.connect(DB_PATH) as conn:
        conn.execute(
            """UPDATE features SET is_enabled = 1 WHERE chat_id = ? AND feature_name = ?""",
            (chat_id, feature_name),
        )
...
def disable_feature(chat_id: int, feature_name: str):
    with sqlite3.connect(DB_PATH) as conn:
        conn.execute(
            """UPDATE features SET is_enabled = 0 WHERE chat_id = ? AND feature_name = ?""",
            (chat_id, feature_name),
        )
...
def update_user_warns(user_id, chat_id, reason):
    ...
    cursor.execute(
        """SELECT history, warns FROM users WHERE user_id = ? AND chat_id = ?""",
        (user_id, chat_id),
    )
    result = cursor.fetchone()
    ...
    warns += 1
    cursor.execute(
        """UPDATE users SET history = ?, warns = ? WHERE user_id = ? AND chat_id = ?""",
        (json.dumps(history), warns, user_id, chat_id),
    )
    conn.commit()
    conn.close()
...
def update_user_bans(user_id, chat_id, reason):
    ...
    cursor.execute(
        """SELECT history, bans FROM users WHERE user_id = ? AND chat_id = ?""",
        (user_id, chat_id),
    )
    result = cursor.fetchone()
    ...
    bans += 1
    cursor.execute(
        """UPDATE users SET history = ?, bans = ? WHERE user_id = ? AND chat_id = ?""",
        (json.dumps(history), bans, user_id, chat_id),
    )
    conn.commit()
    conn.close()
...
def update_user_mutes(user_id, chat_id, reason):
    ...
    cursor.execute(
        """SELECT history, mutes FROM users WHERE user_id = ? AND chat_id = ?""",
        (user_id, chat_id),
    )
    result = cursor.fetchone()
    ...
    mutes += 1
    cursor.execute(
        """UPDATE users SET history = ?, mutes = ? WHERE user_id = ? AND chat_id = ?""",
        (json.dumps(history), mutes, user_id, chat_id),
    )
    conn.commit()
    conn.close()
...
def get_history(user_id, chat_id):
    ...
    cursor.execute(
        """SELECT history FROM users WHERE user_id = ? AND chat_id = ?""",
        (user_id, chat_id),
    )
    result = cursor.fetchone()
    conn.close()
...
def update_user_warn_limit(user_id, chat_id, limit):
    ...
    cursor = conn.cursor()
    cursor.execute(
        """UPDATE users SET warn_limit = warn_limit + ? WHERE user_id = ? AND chat_id = ?""",
        (limit, user_id, chat_id),
    )
    conn.commit()
    conn.close()
...
def update_rep(user_id, chat_id, mode, value=None):
    ...
    if mode not in ["auto_add", "manual_add", "manual_rem"]:
        raise ValueError(
            f"Режим {mode} некорректен, доступные режимы: auto_add, manual_add, manual_rem."
        )
    if mode in ["manual_add", "manual_rem"] and (
        not value or not isinstance(value, int)
    ):
        raise ValueError(
            "Для режимов manual_add и manual_rem необходимо указать целое значение для value."
        )
    ...
    if mode == "auto_add":
        cursor.execute(
            """UPDATE users SET reputation = reputation + ? WHERE user_id = ? AND chat_id = ?""",
            (value, user_id, chat_id),
        )
...
def update_need_msg(user_id, chat_id, count_msg):
    ...
    cursor = conn.cursor()
    cursor.execute(
        """UPDATE users SET message_count = ? WHERE user_id = ? AND chat_id = ?""",
