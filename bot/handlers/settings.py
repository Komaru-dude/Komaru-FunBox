...
async def cmd_settings(message: Message, db: Database):
    ...
    await message.reply(
        ...
        reply_markup=kb_settings.main_settings_keyboard(), parse_mode=ParseMode.HTML
    )
...
async def open_setting(callback: CallbackQuery, db: Database):
    ...
    await callback.message.edit_text(
        ...
        reply_markup=kb_settings.setting_options_keyboard(setting_name, current_value), parse_mode=ParseMode.HTML
    )
    await callback.answer()
...
async def change_int_setting(callback: CallbackQuery, db: Database):
    ...
    # Применяем изменение
    if change == "+1": new_value = current_value + 1
    elif change == "-1": new_value = max(0, current_value - 1)
...
async def request_input(callback: CallbackQuery, state: FSMContext):
    input_type, setting_name = callback.data.split(":")[0].split("_")[1], callback.data.split(":")[1]
