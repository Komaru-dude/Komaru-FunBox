    ...
    try:
        ...
        builder.add(
            InlineKeyboardButton(
                text="✅ Да", callback_data=f"rpconfirm_Yes_{msg_id}_{user_id}"
            ),
            InlineKeyboardButton(
                text="❌ Нет", callback_data=f"rpconfirm_No_{msg_id}_{user_id}"
            ),
        )
    ...
    try:
        ...
        if callback.from_user.id != user_id:
            await callback.answer(
                "⛔️ Кошечка плап запрещает тыкать куда не надо!", show_alert=True
            )
            return

        if callback.message.reply_to_message.message_id != original_msg_id:
            await callback.answer(
                "⚠️ Действие относится к другому сообщению!", show_alert=True
            )
            return
        ...
        if action == "Yes":
            ...
            await callback.message.reply(
                "✅ RP-команды успешно сброшены до базовых настроек!"
            )
        else:
...
async def process_command_name(message: Message, state: FSMContext):
    ...
    if command_name in cur_commands:
        await message.reply(
            "⚠️ Такая команда уже существует. Придумайте другое название:"
        )
        return
...
async def process_messages(message: Message, state: FSMContext):
    ...
    if len(messages_list) > 4:
        await message.reply(
            "⚠️ Можно добавить не более 4 вариантов. Сохраняем первые 4."
        )
