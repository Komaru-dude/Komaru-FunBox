...
async def check_permission(pool: Pool, owner_id: int, user_id: int, chat_id: int, required_level: int) -> bool:
    if user_id == owner_id:
        return True
    async with pool.acquire() as conn:
...
async def modify_reputation(pool: Pool, user_id: int, chat_id: int, mode: str, value: Optional[int] = 0):
    ...
    if mode == "auto_add":
        ...
        if min_rep is None: min_rep = 1
        if max_rep is None: max_rep = 4
...
async def add_history_record(pool: Pool, user_id: int, chat_id: int, punishment_type: str, reason: str):
    async with pool.acquire() as conn:
