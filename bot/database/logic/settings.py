...
async def toggle_chat_val(pool: Pool, chat_id: int, name: str, enable: Optional[bool] = None) -> bool:
...
async def get_user_val(pool: Pool, user_id: int, name: str):
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow("SELECT settings FROM global_users WHERE user_id = $1", user_id)
        
        settings_raw = row.get("settings") if row else None
        
        # Парсинг JSON
        if isinstance(settings_raw, str):
            try: settings = json.loads(settings_raw)
            except: settings = {}
