...
async def get_username_by_id(chat_id: str, user_id: int):
    ...
    try:
        ...
        logging.error(f"Ошибка при получении username для {user_id} в чате {chat_id}: {e}")
        raise HTTPException(status_code=400, detail=str(e))
...
async def get_first_name_by_id(chat_id: str, user_id: int):
    ...
    try:
        ...
        logging.error(f"Ошибка при получении first_name для {user_id} в чате {chat_id}: {e}")
        raise HTTPException(status_code=400, detail=str(e))
